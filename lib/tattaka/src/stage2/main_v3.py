...
class BirdClef2021Dataset(Dataset):
    def __init__(
        ...
        pseudo_label_path: list = [
            ...
            "/cs/home/psykjm/input/birdclef-2021/pseudo_label_stage2_resnet34",
        ],
        period: float = 15.0,
    ...
    def __getitem__(self, idx):
        ...
        if self.duration[idx] > self.period * 6:
            # clipの長さが学習に用いる秒数*6より長い場合のみ、secondary labelを小さくする
            secondary_coef = 0.1
        for s in self.secondary_labels[idx]:
            if s == "rocpig1":
                s = "rocpig"
            if s != "" and s in bird2id.keys():
                target[bird2id[s]] = secondary_coef
